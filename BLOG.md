...
To keep Nginx on the droplet fronting everything through port 8000 I remapped the compose service so host port 8000 forwards to Gunicorn’s new 9000 listener and pointed the container health check at `/health` on that port, which prevents the compose watchdog from flapping now that Gunicorn is bound to loopback.
